...
function LoginPage({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(false);
    if (!ok) setError(lang === 'zh' ? '帳號或密碼錯誤，或帳號已停用' : 'Invalid credentials or account disabled');
    else onLogin?.();
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: TOKENS.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, backgroundImage: `radial-gradient(circle at 20% 30%, rgba(0,212,170,0.06) 0%, transparent 50%), radial-gradient(circle at 80% 70%, rgba(64,144,240,0.06) 0%, transparent 50%)`, pointerEvents: 'none' }}></div>
      <div style={{ position: 'absolute', inset: 0, backgroundImage: 'linear-gradient(rgba(30,37,64,0.3) 1px, transparent 1px), linear-gradient(90deg, rgba(30,37,64,0.3) 1px, transparent 1px)', backgroundSize: '40px 40px', pointerEvents: 'none', opacity: 0.5 }}></div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <input value={username} onChange={e => { setUsername(e.target.value); setError(''); }} placeholder={lang === 'zh' ? '輸入帳號' : 'Enter username'} autoComplete="username"
                style={{ padding: '10px 12px', background: TOKENS.bgInput, border: `1px solid ${error ? TOKENS.danger : TOKENS.border}`, borderRadius: TOKENS.radiusSm, color: TOKENS.text, fontSize: 14, fontFamily: TOKENS.font, outline: 'none', transition: 'border-color 0.15s' }}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <div style={{ position: 'relative' }}>
                <input value={password} onChange={e => { setPassword(e.target.value); setError(''); }} type={showPw ? 'text' : 'password'} placeholder={lang === 'zh' ? '輸入密碼' : 'Enter password'} autoComplete="current-password"
                  style={{ width: '100%', padding: '10px 40px 10px 12px', background: TOKENS.bgInput, border: `1px solid ${error ? TOKENS.danger : TOKENS.border}`, borderRadius: TOKENS.radiusSm, color: TOKENS.text, fontSize: 14, fontFamily: TOKENS.font, outline: 'none', transition: 'border-color 0.15s' }}
                  onFocus={e => e.target.style.borderColor = TOKENS.borderFocus} onBlur={e => e.target.style.borderColor = error ? TOKENS.danger : TOKENS.border} />
                <button type="button" onClick={() => setShowPw(!showPw)} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: TOKENS.textMuted, cursor: 'pointer', fontSize: 13, padding: 2 }}>
                  {showPw ? '👁' : '👁‍🗨'}
                </button>
              </div>
            </div>
            ...
            <button type="submit" disabled={loading}
              style={{ padding: '11px', background: loading ? TOKENS.border : TOKENS.primary, border: 'none', borderRadius: TOKENS.radiusSm, color: loading ? TOKENS.textMuted : '#0a0e1a', fontSize: 14, fontWeight: 700, fontFamily: TOKENS.font, cursor: loading ? 'not-allowed' : 'pointer', transition: 'background 0.15s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              {loading ? (
                <><div style={{ width: 16, height: 16, border: '2px solid rgba(255,255,255,0.3)', borderTop: '2px solid #fff', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }}></div>{lang === 'zh' ? '登入中...' : 'Signing in...'}</>
              ) : (lang === 'zh' ? '登入' : 'Sign In')}
            </button>
          </form>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {demoAccounts.map(a => (
              <button key={a.username} onClick={() => { setUsername(a.username); setPassword(a.password); setError(''); }}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', background: TOKENS.bgInput, border: `1px solid ${TOKENS.border}`, borderRadius: TOKENS.radiusSm, cursor: 'pointer', transition: 'border-color 0.15s', fontFamily: TOKENS.font }}
                onMouseEnter={e => e.currentTarget.style.borderColor = TOKENS.borderFocus} onMouseLeave={e => e.currentTarget.style.borderColor = TOKENS.border}>
